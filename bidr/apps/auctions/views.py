...
class AuctionView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["upcoming_auctions"] = Auction.objects.filter(stage=STAGES.index("Plan")).orderedby("start_time")
        context["current_auctions"] = Auction.objects.filter(stage=STAGES.index("Observe")).orderedby("start_time")
        context["complete_auctions"] = Auction.objects.filter(stage__gte=STAGES.index("Claim")).orderedby("start_time")
